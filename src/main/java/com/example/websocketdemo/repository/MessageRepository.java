...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT m FROM Message m WHERE m.chat.id = :chatId AND m.id > :lastReadMessageId ORDER BY m.createdAt ASC")
    List<Message> findUnreadMessages(@Param("chatId") Long chatId, @Param("lastReadMessageId") Long lastReadMessageId);
    
    @Query("SELECT m FROM Message m WHERE m.chat.id = :chatId AND m.senderUsername != :username AND m.readAt IS NULL ORDER BY m.createdAt ASC")
    List<Message> findUnreadMessagesForUser(@Param("chatId") Long chatId, @Param("username") String username);
    
    @Modifying
    @Query("UPDATE Message m SET m.readAt = :readAt WHERE m.chat.id = :chatId AND m.senderUsername != :username AND m.readAt IS NULL")
